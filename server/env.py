...
class SupportEnv(Environment):
    async def reset(self) -> StepResult:
        ...
        self.current_obs = SupportObservation(ticket_text=self.ticket, tool_output="None", step_count=0)
    ...
    async def step(self, action: SupportAction) -> StepResult:
        ...
        if action.tool_name == "search_kb":
            ...
            if self.current_task == "angry_escalation" and dept == "TechSupport":
                reward += 0.8
                done = True
            elif self.current_task == "payment_issue" and dept == "Billing":
                reward += 0.8
                done = True
            else:
            ...
            if self.current_task == "hard_policy_enforcement":
                if self.has_checked_db and self.has_checked_kb:
                    if "30" in str(action.tool_args) or "policy" in str(action.tool_args).lower():
                        reward += 0.8
                        done = True
                    else:
        ...
        self.current_obs = SupportObservation(ticket_text=self.ticket, tool_output=tool_output, step_count=self.step_count)
        return StepResult(observation=self.current_obs, reward=reward, done=done)

    async def state(self) -> SupportObservation:
